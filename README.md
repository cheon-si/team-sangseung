...
| 예약 | 매일 14:00~21:00 KST 매시 정각, 8개. GitHub 예약이 4시간 15분~6시간 39분 늦게 뜨고(9/17~9/24 실측) 폭이 매일 달라 여러 개를 건다. 창 시작 1.5시간 전보다 일찍 뜬 실행은 바로 종료, 그 안에 뜨면 기다렸다 수집. 02:00~12:00에 뜬 실행은 어젯밤 수집분이 없을 때만 실패로 표시. 수집 중에 뜬 실행은 대기열에서 취소(회색)되거나 02:00 이후 바로 종료되므로 정상 |
